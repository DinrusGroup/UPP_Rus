...
void CodeEditor::OpenNormalFindReplace(bool replace)
{
	...
	findreplace.Title(replace ? "Найти и Заменить" : "Найти");
	findreplace.findback.Show();
	findreplace.ok.SetLabel("Найти Далее");
...
void CodeEditor::FindReplace(bool pick_selection, bool pick_text, bool replace)
{
	...
	if(IsSelection() && replace) {
		findreplace.itext = GetI();
		findreplace.Title("Заменить в выделенном");
		findreplace.amend.Hide();
		findreplace.findback.Hide();
		findreplace.ok.SetLabel("Заменить");
...
{
	MenuBar menu;
	menu.Add("Один или более пробелов", callback1(cb, '%'));
	menu.Add("Один или более любых символов", callback1(cb, '*'));
	menu.Add("Идентификатор C++", callback1(cb, '$'));
	menu.Add("Число", callback1(cb, '#'));
	menu.Add("Любой символ", callback1(cb, '?'));
	if(tablf) {
		menu.Separator();
		menu.Add("Таб", callback1(cb, '\t'));
		menu.Add("Лайн-фид", callback1(cb, '\n'));
	}
...
{
	MenuBar menu;
	menu.Add("Совпадающие пробелы", THISBACK1(InsertWildcard, '%'));
	menu.Add("Совпадающие один или более символов", THISBACK1(InsertWildcard, '*'));
	menu.Add("Совпадающий идентификатор C++", THISBACK1(InsertWildcard, '$'));
	menu.Add("Совпадающее число", THISBACK1(InsertWildcard, '#'));
	menu.Add("Совпадающий любой символ", THISBACK1(InsertWildcard, '?'));
	menu.Separator();
	for(int i = 1; i <= 9; i++)
		menu.Add(Format("Совпадающий шаблон %d", i), THISBACK1(InsertWildcard, i));
	menu.Separator();
	menu.Add("Таб", THISBACK1(InsertWildcard, 20));
	menu.Add("Лайн-фид", THISBACK1(InsertWildcard, 21));
	int l, h;
