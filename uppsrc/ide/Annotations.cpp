...
void AssistEditor::SyncAnnotationPopup()
{
	...
	if(tl.GetCount()) {
		...
		annotation_popup.SetQTF("[A1 [@b* " + DeQtf(coderef) + "]&Ещё не документирован - кликните, чтобы документировать");
...
void AssistEditor::EditAnnotation(bool fastedit)
{
	...
	bar.Add("Новая справочная тематика..", THISBACK2(NewTopic, "src", coderef));
	bar.Add("Новая тематика реализации..", THISBACK2(NewTopic, "srcimp", coderef));
	bar.Execute();
}
