...
void WorkspaceWork::AddSeparator()
{
	...
	if(active.IsEmpty()) return;
	String name;
	if(!EditText(name, "Вставьте разделитель", "Имя"))
		return;
...
ImportDlg::ImportDlg()
{
	...
	files <<= "*.cpp *.h *.hpp *.c *.C *.cxx *.cc *.d *.di";
}
...
void WorkspaceWork::DoImportTree(const String& dir, const String& mask, bool sep, Progress& pi, int from)
{
	...
	if(sep && files.GetCount()) {
		if(!DirectoryExists(absPath))
			if(!RealizeDirectory(absPath))
				throw Format("Произошла ошибка при создании папки:&\1%s", absPath);
	...
	for(int i = 0; i < files.GetCount(); i++) {
		...
		if(FileCopy(files[i], AppendFileName(absPath, name))) {
			...
			throw Format("Произошла ошибка при копировании файла:&\1%s", files[i]);
	}
...
void WorkspaceWork::DelFile()
{
	...
	if(IsFolder(file)) {
		if(!PromptYesNo("Удалить группу тематик и ВСЕ тематики из базы?")) return;
		RemoveFile();
		DeleteFolderDeep(file);
	}
	else {
		if(!PromptYesNo("Удалить файл из пакета и базы ?")) return;
		RemoveFile();
		::DeleteFile(file);
	}
}
...
void WorkspaceWork::RenameFile()
{
	...
	if(!filelist[i].isdir && GetFileLength(spath) >= 0) {
		if(!::MoveFile(spath, dpath)) {
			Exclamation("Не удалось переименовать файл.&&" + GetErrorMessage(GetLastError()));
			return;
		}
	}
	FileRename(dpath);
...
{
	bool isaux = IsAux();
	menu.Add("Введите любой файл(-ы)", THISBACK1(AddFile, ANY_FILE))
		.Key(K_SHIFT|K_CTRL_I)
		.Help("Вставьте любые файлы с диска (что плохо для портабельности пакетов...)");
	menu.Add(isaux && GetOutputDir().GetCount(), "Вставьте файл(-ы) папки вывода", THISBACK1(AddFile, OUTPUT_FILE))
		.Help("Открыть файлвыбор в выводной / промежуточной директории текущего пакета");
	menu.Add(isaux, "Вставьте файл(-ы) Локальной папки", THISBACK1(AddFile, LOCAL_FILE))
		.Help("Открыть файлвыбор в Локальной папке текущего пакета");
	menu.Add("Ввведите файл(-ы) домашней папки", THISBACK1(AddFile, HOME_FILE))
		.Help("Открыть файлвыбор в текущей папке HOME пользователя");
}

void WorkspaceWork::SpecialFileMenu(Bar& menu)
{
	InsertSpecialMenu(menu);
	menu.Add("Импортировать исходники дерева папки..", THISBACK(Import));
}
...
void WorkspaceWork::FileMenu(Bar& menu)
{
	...
	if(isaux)
		...
		menu.Add(!isaux, "Введите файл(-ы) из директории пакета", THISBACK1(AddFile, PACKAGE_FILE))
			.Help("Введите файл относительно текущего пакета");
		menu.Add(!isaux, "Введите группу topic++", THISBACK(AddTopicGroup));
		menu.Add("Особое", THISBACK(SpecialFileMenu))
		    .Help("Менее часто используемые методы добавления файлов в пакет");
	}
	menu.Add("Введите разделитель", THISBACK(AddSeparator))
		.Help("Введите разделитель текстовых строк");
	menu.Separator();
	if(!organizer) {
		if(sel)
			menu.Add(closed.Find(GetActiveSepfo()) < 0 ? "Закрыть группу\t[double-click]"
			                                           : "Открыть группу\t[double-click]", THISBACK(Group));
		menu.Add("Открыть все группы", THISBACK(OpenAllGroups));
		menu.Add("Закрыть все группы", THISBACK(CloseAllGroups));
		menu.Separator();
		BuildFileMenu(menu);
		menu.Separator();
	}
	menu.Add("Переименовать...", THISBACK(RenameFile))
	    .Help("Переименовать файл / разделитель / группу тематик");
	menu.Add("Удалить", THISBACK(RemoveFile))
		.Key(organizer ? K_DELETE : K_ALT_DELETE)
		.Help("Удалить файл / разделитель / группу тематик из пакета");
	menu.Add(filelist.IsCursor() && !sel, "Стереть", THISBACK(DelFile))
		.Help("Удалить файл / ссылку на группу тематик из пакета & стереть файл / папку на диске");
	menu.Separator();
	menu.Add("Открыть Папку Файла",THISBACK(OpenFileFolder));
	menu.Add("Копировать Путь К Файлу", callback1(WriteClipboardText, GetActiveFilePath()));
	menu.Separator();
	menu.Add(filelist.GetCursor() > 0, "Поднять", THISBACK1(MoveFile, -1))
		.Key(organizer ? K_CTRL_UP : K_SHIFT_CTRL_UP)
		.Help("Переместить текущий файл на позицию выше к началу пакета");
	menu.Add(filelist.IsCursor() && filelist.GetCursor() < filelist.GetCount() - 1, "Опустить",
	         THISBACK1(MoveFile, 1))
		.Key(organizer ? K_CTRL_DOWN : K_SHIFT_CTRL_DOWN)
		.Help("Переместить текущий файл на позицию ниже к концу пакета");
	if(IsActiveFile()) {
		menu.Separator();
		String p = GetActiveFilePath();
		if(GetFileExt(p) == ".tpp" && IsFolder(p)) {
			menu.Add("Включаемая группа тематик", THISBACK(ToggleIncludeable))
			    .Check(FileExists(AppendFileName(p, "all.i")));
			if(IsSvnDir(p))
				menu.Add("Svn Синхронизация " + p, THISBACK1(SyncSvnDir, p));
		}
		else
			menu.Add("Оптимизировать на скорость", THISBACK(ToggleFileSpeed))
...
{
	String p = SelectPackage("Выберите пакет");
	if(p.IsEmpty()) return;
	...
	for(int i = 0; i < package.GetCount(); i++) {
		...
		for(int i = 0; i < prj.uses.GetCount(); i++)
			if(UnixPath(prj.uses[i].text) == active) {
				usecnt++;
				bar.Add("Удалить из '" + pn + '\'', THISBACK1(RemovePackage, pn))
					.Help(NFormat("Удалить пакет '%s' из рездела использований в '%s'", active, pp));
			}
	}
	if(usecnt > 1) {
		bar.MenuSeparator();
		bar.Add("Удалить все использования", THISBACK1(RemovePackage, String(Null)))
			.Help(NFormat("Удалить пакет '%s' из всех использований в активном проекте и подмодулях", active));
	}
}
...
void WorkspaceWork::RemovePackage(String from_package)
{
	...
	if(IsNull(from_package) && !PromptYesNo(NFormat(
		"Удалить пакет [* \1%s\1] из раздела использований всех текущих пакетов ?", active)))
		return;
	for(int i = 0; i < package.GetCount(); i++)
...
void WorkspaceWork::PackageMenu(Bar& menu)
{
	if(!menu.IsScanKeys()) {
		...
		menu.Add(cando, ~NFormat("Добавить пакет в '%s'", act), IdeImg::package_add(), THISBACK(AddNormalUses));
		RemovePackageMenu(menu);
		if(menu.IsMenuBar()) {
			menu.Separator();
			menu.Add(cando, "Оптимизировать на скорость", THISBACK(TogglePackageSpeed))
