...
{
	CtrlLayout(*this);
	package.AddColumn("Пакет");
	package.AddColumn("Отладка").Ctrls(DebugF);
	package.AddColumn("Блиц").Ctrls(BlitzF);
...
void OutMode::Export(int kind)
{
	...
	if(IsNull(ep)) {
		Exclamation("Отсутствует папка для вывода!");
		return;
	}
	if(kind == 2)
...
void Ide::SyncBuildMode()
{
	...
	String h = method + ' ';
	switch(targetmode) {
	case 1: h << "Оптимально"; break;
	case 2: h << "Скорость"; break;
	case 3: h << "Размер"; break;
	default:
		h << "Отладка";
	}
	buildmode <<= h;
