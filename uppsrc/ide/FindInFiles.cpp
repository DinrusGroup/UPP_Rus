...
bool Ide::SearchInFile(const String& fn, const String& pattern, bool wholeword, bool ignorecase,
	...
	if(infile && ffs != STYLE_NO_REPLACE)
	{
		...
		if(ffs == STYLE_CONFIRM_REPLACE)
		{
			...
			switch(PromptYesNoCancel(NFormat("Заменить %d строки в [* \1%s\1]?", infile, fn)))
			{
			case 1:  break;
		...
		if(doit)
		{
			...
			editor.BlockReplace();
			SaveFile();
			console2 << NFormat("%s: %d замен выполнено\n", fn, infile);
			console2.Sync();
		}
	}
...
void Ide::FindFileName() {
	...
	CtrlLayoutOKCancel(ffdlg, "Найти файл");
	ffdlg.list.AutoHideSb();
	ffdlg.list.AddColumn("Пакет");
	ffdlg.list.AddColumn("Файл");
	ffdlg.list.WhenLeftDouble = ffdlg.Acceptor(IDOK);
	ffdlg.mask.NullText("Поиск");
...
void Ide::FindInFiles(bool replace) {
	...
	if(c == IDOK && !String(ff.find).IsEmpty()) {
		...
		Progress pi("Найдено %d файлов при поиске.");
		Vector<String> files;
		...
		if(!pi.Canceled()) {
			...
			console2 << Format("%d совпадений обнаружено.\n", n);
		}
	}
	SetErrorEditor();
}
...
void Ide::ConstructFindInFiles() {
	...
	static const char *defs = "*.cpp *.h *.hpp *.c *.m *.C *.M *.cxx *.cc *.d *.di *.mm *.MM *.icpp *.sch *.lay *.rc";
	ff.files <<= String(defs);
	ff.files.AddList((String)defs);
	ff.files.AddList((String)"*.txt");
	ff.files.AddList((String)"*.*");
	ff.folder.AddButton().SetMonoImage(CtrlImg::smalldown()).Tip("Связанные папки") <<= THISBACK(FindStdDir);
	ff.folder.AddButton().SetMonoImage(CtrlImg::smallright()).Tip("Выберите папку") <<= THISBACK(FindFolder);
	editor.PutI(ff.find);
...
{
	Title(replacing ? "Найти и заменить в файлах" : "Найти в файлах");
