...
String ResolveTParam(const String& type, const Vector<String>& tparam)
{
	...
	LLOG("Разобран " << type << " -> " << r);
	const Array<CppItem>& x = GetTypeItems(r);
	if(x.GetCount() && x[0].kind == TYPEDEF) {
		LLOG("Является typedef " << x[0].qtype << ';' << x[0].type << ';' << x[0].natural);
		String h = x[0].qtype;
		if(h != type && h != r)
...
#ifdef _DEBUG_X_0
	PutVerbose("тело: " + AsString(inbody));
	PutVerbose("масштаб: " + AsString(parser.current_scope));
#endif
}
                                  ...
                                  bool can_shortcut_operator, Index<String>& visited_bases)
{
	...
	LLOG("ExpressionType " << type << " ii: " << ii << " id:" << id << " переменная:" << variable);
...
Index<String> AssistEditor::ExpressionType(const Parser& parser, const Vector<String>& xp)
{
	...
	if(q >= 0) {
		...
		LLOG("Найден локальный тип: " << type << " в масштабе: " << parser.current_scope);
	...
	if(xp.GetCount() >= 2 && xp[1] == "()") {
		...
		if(CodeBase().Find(ParseTemplatedType(qtype, tparam)) >= 0) {
			LLOG("Является конструктором " << qtype);
