...
void Pdb::MemoryGoto(const String& exp)
{
	...
	try {
		...
		Exclamation("Неверное выражение!&" + DeQtf(e));
	}
}
	...
	try {
		...
		if(v.ref > 0) {
			if(sep)
				bar.Separator();
			sep = false;
			bar.Add("Память по " + exp, THISBACK1(MemoryGoto, exp));
		}
		else
		if(v.rvalue) {
			if(sep)
				bar.Separator();
			sep = false;
			bar.Add("Память по 0x" + FormatIntHex((dword)GetInt(v)), THISBACK1(MemoryGoto, "&" + exp));
		}
		if(!v.rvalue) {
			if(sep)
				bar.Separator();
...
{
	bar.Add("Исследовать", THISBACK1(ExploreKey, &array));
...
{
	bar.Add("Исследовать", THISBACK(ExpExp));
