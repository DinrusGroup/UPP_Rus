...
{
	bar.Add("Остановить отладку", THISBACK(Stop))
	   .Key(K_SHIFT_F5);
	bar.Separator();
	bool b = !IdeIsDebugLock();
	bar.Add(b, "Вступить", DbgImg::StepInto(), THISBACK1(Step, disas.HasFocus() ? "stepi"
	                                                                             : "step"))
	   .Key(K_F11);
	bar.Add(b, "Перступить", DbgImg::StepOver(), THISBACK1(Step, disas.HasFocus() ? "nexti"
	                                                                             : "next"))
	   .Key(K_F10);
	bar.Add(b, "Выступить", DbgImg::StepOut(), THISBACK1(Step, "finish"))
	   .Key(K_SHIFT_F11);
	bar.Add(b, "Выполнить до", DbgImg::RunTo(), THISBACK(DoRunTo))
	   .Key(K_CTRL_F10);
	bar.Add(b, "Выполнить", DbgImg::Run(), THISBACK(Run))
	   .Key(K_F5);
	bar.Separator();
	bar.Add(b, "Быстрый просмотр", THISBACK(QuickWatch))
	   .Key(K_CTRL_Q);
	bar.MenuSeparator();
	bar.Add(b, "Копировать бэктрейс", THISBACK(CopyStack));
	bar.Add(b, "Коптровать дизасм", THISBACK(CopyDisas));
}
...
{
	return FindTag(FastCmd(text), "Останов");
}
...
void Gdb::CheckEnd(const char *s)
{
	...
	if(FindTag(s, "Программа завершилась нормально.")) {
		Stop();
		return;
	}
	const char *q = FindTag(s, "Программа завершилась с кодом ");
	if(q) {
		PutConsole(q);
		Stop();
		return;
...
bool Gdb::Create(One<Host> _host, const String& exefile, const String& cmdline)
{
	...
	if(!dbg) {
		Exclamation("Не удалось вызвать gdb !");
		return false;
	}
	IdeSetBottom(*this);
	IdeSetRight(disas);
