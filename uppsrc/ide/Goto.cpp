...
*/
	list.HeaderTab(0).SetText(Format("Масштаб (%d)", nc.GetCount()));
	list.HeaderTab(1).SetText(Format("Символ (%d)", list.GetCount()));
	SyncOk();
}
...
GotoDlg::GotoDlg(const String& s)
{
	...
	CtrlLayoutOKCancel(*this, IsNull(s) ? "Перейти к глобальной" : "Перейти к строке или символу");
	...
	list.AddColumn("Нестинг");
	list.AddColumn().SetDisplay(Single<CppItemInfoDisplay>());
	list.AddColumn(global ? "Позиция" : "Строка");
...
bool Ide::SwapSIf(const char *cref)
{
	...
	LLOG("Масштаб SwapS: " << p.current_scope);
	if(q < 0)
		return false;
	...
	LLOG("Строка SwapS: " << line);
	int i;
	for(i = 0; i < count; i++) {
		LLOG("файл: " << GetCppFile(n[q + i].file) << ", строка: " << n[q + i].line);
