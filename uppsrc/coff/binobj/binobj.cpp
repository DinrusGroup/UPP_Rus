		...
		if(binid == "BINARY" || ba || bm) {
			...
			if(ba) {
				...
				if(blk.index < 0 || blk.index > 1000000)
					binscript.ThrowError(NFormat("неверный индекс массива: %d", blk.index));
				binscript.PassChar(',');
			}
			else if(bm) {
			...
			if(ff.Search(searchpath))
				...
				while(ff.Next());
			if(files.IsEmpty())
				binscript.ThrowError(NFormat("'%s' не обнаружен, либо не является файлом", file));
			if(!(blk.flags & Block::FLG_MASK) && files.GetCount() > 1)
				binscript.ThrowError(NFormat("Обнаружено несколько файлов (напр. %s, %s) в одном файловом режиме", files[0], files[1]));
			...
			for(int i = 0; i < files.GetCount(); i++) {
				...
				if(f >= 0)
					binscript.ThrowError(NFormat("%s[%d] уже виден на строке %d", blk.ident, blk.index, brow[f].scriptline));
				if(blk.index < 0 && !brow.IsEmpty() || blk.index >= 0 && !brow.IsEmpty() && brow[0].index < 0)
					binscript.ThrowError(NFormat("%s: смешение элементов массива и немассива", blk.ident));
			...
			binscript.ThrowError("ожидался двоичный идентификатор элемента сценария");
	}
}
...
void BinObj::WriteFile(String objectfile)
{
	...
	if(!fo.Open(objectfile))
		throw Exc(NFormat("ошибка при создании файла '%s'", objectfile));
		...
		for(int i = 0; i < blocks.GetCount(); i++) {
			...
			for(int a = 0; a < belem.GetCount(); a++) {
				...
				if(data.GetLength() != b.length)
					throw Exc(NFormat("длина файла '%s' изменилась (%d -> %d) при создании файла",
	...
	fo.Close();
	if(fo.IsError())
		throw Exc(NFormat("ошибка при записи файла '%s'", objectfile));
}
...
static void TryMain()
{
	...
	for(int i = 0; i < cmdline.GetCount(); i++) {
		...
		if(IsNull(binfile))
			...
			throw Exc(NFormat("аргумент '%s' не может быть использован", arg));
	}
	if(IsNull(binfile)) {
		puts(Usage());
		SetExitCode(-1);
	...
	if(bindata.IsVoid())
		throw Exc(NFormat("ошибка при чтении файла '%s'", binfile));
