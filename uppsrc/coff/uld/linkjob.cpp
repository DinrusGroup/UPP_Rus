...
void LinkJob::ReadCommand(const Vector<String>& cmdlist, bool user)
{
	...
	for(int i = 0; i < cmdlist.GetCount(); i++) {
		...
		if(is_opt || *arg == '-') {
			...
			if(!eaten)
				PutStdOut(NFormat("Неизвестная опция: %s", arg));
		}
		else
...
//		cache_file_name = ForceExt(outputfile, ".uld");
	...
	if(!mapping.IsOpen()) {
		PutStdOut(NFormat("Файл '%s' не найден.", fn));
		return;
	}
...
//	loaded_files.Add(full);
	...
	PutStdOut(NFormat("Файл '%s' неизвестного типа.", file));
}
...
void LinkJob::LoadObject(String file, const FileMapping& mapping, ObjectFile::FILETYPE filetype)
{
	...
	if(verbose)
		PutStdOut(NFormat("%s: загружается объектный файл", file));
...
{
	if(verbose)
		PutStdOut(NFormat("%s: загружается статическая библиотека", file));
