...
{
	if(archive.verbose)
		PutStdOut(NFormat("%s: чтение файла объекта (%d B)", filename, object_data.GetLength()));
...
void ArchiveJob::LoadFile(String file)
{
	...
	if(!mapping.Open(file))
		throw Exc(NFormat("%s: не удалось открыть файл", file));
...
void ArchiveJob::LoadLibrary(String libfile)
{
	...
	if(!mapping.Open(libfile))
	{
		if(archive_must_exist)
			throw Exc(NFormat("%s: не удалось открыть файл архива", libfile));
		if(use_existing_archive && verbose)
			PutStdOut(NFormat("%s: archive not found", libfile));
		return;
	}
	if(memcmp(~mapping, "!<arch>\n", 8))
		throw Exc(NFormat("%s: недействительный файл библиотеки (отсутствует !<arch>\\n заголовок)", libfile));
	if(verbose)
