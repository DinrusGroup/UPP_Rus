...
{
	bar.Add(!IsReadOnly(), t_("Подчёркнутый"),
...
{
	bar.Add(!IsReadOnly(), t_("Зачёркнутый"),
...
{
	bar.Add(t_("Показать ошибки написания"), RichEditImg::SpellCheck(), USERBACK(SpellCheck))
	   .Check(spellcheck);
}
...
void RichEdit::IndexEntryTool(Bar& bar, int width, dword key, const char *tip)
{
	...
	Setup(indexentry, tip, t_("Индексировать запись"), key);
...
void RichEdit::HyperlinkTool(Bar& bar, int width, dword key, const char *tip)
{
	...
	Setup(hyperlink, tip, t_("Гипрссылка"), key);
...
void RichEdit::LabelTool(Bar& bar, int width, dword key, const char *tip)
{
	...
	Setup(label, tip, t_("Метка параграфа"), key);
...
void RichEdit::LeftTool(Bar& bar, dword key)
{
	...
	bar.Add(!IsReadOnly(), t_("Влево"), RichEditImg::Left(), USERBACK(AlignLeft))
	   .Check(a == ALIGN_LEFT)
	   .Key(key);
}
...
{
	int a = formatinfo.paravalid & RichText::ALIGN ? formatinfo.align : Null;
	bar.Add(!IsReadOnly(), t_("Вправо"), RichEditImg::Right(), USERBACK(AlignRight))
	   .Check(a == ALIGN_RIGHT)
	   .Key(key);
}
...
{
	bar.Add(!IsReadOnly() && undo.GetCount(), t_("Откат"), CtrlImg::undo(), USERBACK(Undo))
	   .Repeat()
	   .Key(K_ALT_BACKSPACE)
	   .Key(key);
}
...
{
	bar.Add(!IsReadOnly() && redo.GetCount(), t_("Накат"), CtrlImg::redo(), USERBACK(Redo))
	   .Repeat()
...
{
	bar.Add(!IsReadOnly(), t_("Вставить рисунок из файла.."), RichEditImg::LoadImageFile(), THISBACK(LoadImage));
}
...
void RichEdit::SplitJoinCellTool(Bar& bar, dword key)
{
	if(tablesel)
		...
		bar.Add(!IsSelection() && cursorp.table && !IsReadOnly(), t_("Разбить ячейку.."),
