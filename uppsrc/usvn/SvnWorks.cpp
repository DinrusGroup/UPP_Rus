...
void SvnWorks::New()
{
	...
	CtrlLayoutOKCancel(dlg, "Добавить рабочую папку SVN");
	DirSel(dlg.working);
	if(dlg.Execute() != IDOK)
		return;
...
void SvnWorks::Edit()
{
	if(!list.IsCursor())
		return;
	WithSvnFolderLayout<TopWindow> dlg;
	CtrlLayoutOKCancel(dlg, "Редактировать элемент рабочей папки SVN");
	DirSel(dlg.working);
...
void SvnWorks::Checkout()
{
	...
	CtrlLayoutOKCancel(dlg, "Копировать репозиторий SVN");
	DirSel(dlg.working);
	if(dlg.Execute() != IDOK)
		return;
	...
	if(working.GetCount() == 0) {
		Exclamation("Пустая рабочая папка");
		return;
	}
...
{
	if(list.IsCursor() && PromptYesNo("Удалить элемент?"))
...
SvnWorks::SvnWorks()
{
	CtrlLayoutExit(*this, "Рабочие папки SVN");
	list.AddColumn("Рабочая папка");
	list.AddColumn("Пользователь");
	list.AddColumn("Пароль");
	list.AddColumn("Только чтение");
