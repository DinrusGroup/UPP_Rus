...
void TextCtrl::StdBar(Bar& menu) {
	NextUndo();
	if(undosteps) {
		menu.Add(undo.GetCount() && IsEditable(), t_("Отменить"), THISBACK(Undo))
			.Key(K_ALT_BACKSPACE)
			.Key(K_CTRL_Z);
		menu.Add(redo.GetCount() && IsEditable(), t_("Повторить"), THISBACK(Redo))
	...
	menu.Separator();
	menu.Add(GetLength(),
			t_("Выбрать все"), THISBACK(SelectAll))
		.Key(K_CTRL_A);
}
